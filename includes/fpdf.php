...
class FPDF {
    ...
    function AddFont($family, $style='', $file='') {
        ...
        if(in_array($family, $this->CoreFonts)) {
            $info = $this->_loadcorefont($family, $style);
        } else {
    ...
    protected function _loadcorefont($family, $style) {
        $fontkey = $family.$style;
        $fontfile = __DIR__ . '/' . str_replace(' ','',$family).strtolower($style).'.php';
        
        if(file_exists($fontfile)) {
            include($fontfile);
            return array('type'=>'core', 'name'=>$name, 'up'=>$up, 'ut'=>$ut, 'cw'=>$cw);
        }
    ...
    function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='') {
        ...
        $this->_out(sprintf('q %.2F 0 0 %.2F %.2F %.2F cm /I%d Do Q',$w*$k,$h*$k,$x*$k,($this->h-($y+$h))*$k,$info['i']));
        if($link)
    ...
    protected function _dochecks() {
        if(PHP_VERSION_ID<50300)
            $this->Error('FPDF requires PHP 5.3 or above');
    }
    ...
    protected function _putpage($n) {
        ...
        if(isset($this->PageLinks[$n])) {
            ...
            foreach($this->PageLinks[$n] as $pl) {
                ...
                if(is_string($pl[4]))
                    ...
                    $annots .= sprintf('/Dest [%d 0 R /XYZ 0 %.2F null]>>',2*$l[0]-1,$h-$l[1]*$this->k);
                }
            }
            $this->_out($annots.']');
        }
        ...
        if($this->compress) {
            ...
            $this->_out('<<'.$this->_getfilter().' /Length '.strlen($p).'>>');
        } else {
            $p = $this->pages[$n];
    ...
    protected function _putimage(&$info) {
        ...
        if(isset($info['smask'])) {
            ...
            $smask = array('w'=>$info['w'], 'h'=>$info['h'], 'cs'=>'DeviceGray', 'bpc'=>$info['bpc'], 'f'=>$info['f'], 'dp'=>$dp, 'data'=>$info['smask']);
            $this->_putimage($smask);
        }
        if($info['cs']=='Indexed') {
            $this->_newobj();
            $pal = ($this->compress) ? gzcompress($info['pal']) : $info['pal'];
            $this->_out('<<'.$this->_getfilter().' /Length '.strlen($pal).'>>');
            $this->_putstream($pal);
            $this->_out('endobj');
        }
    }
